...
class OpenGLWidget(QOpenGLWidget):
    ...
    def _process_geometry(self):
        ...
        for point in self.coords:
            if len(point) == 2:
                ...
                coords_3d.append(point[:3])  # Tomar solo x,y,z
        
        self.coords_array = np.array(coords_3d, dtype=np.float32)
        
        # Procesar elementos en triángulos
        triangles = []
        edges = set()  # Para líneas
        
        for elem in self.elements:
            elem_triangles = self._element_to_triangles(elem)
            triangles.extend(elem_triangles)
            
            # Generar aristas para líneas
    ...
    def _element_to_triangles(self, elem):
        ...
        if elem_len == 3:  # Triángulo
            ...
            return self._tetrahedron_faces(elem) if self._is_3d_element(elem) else [[elem[0], elem[1], elem[2]], [elem[0], elem[2], elem[3]]]
    ...
    def _is_3d_element(self, elem):
        ...
        return volume > 1e-10  # Tolerancia para elementos planos
    ...
    def _hexahedron_faces(self, elem):
        """Caras de un hexaedro (orden estándar de nodos)"""
        return [
    ...
    def _render_wireframe(self, mvp_matrix):
        ...
        shader = self.shader_programs.get("line")
        if not shader:
            return
    ...
    def paintGL(self):
        ...
        mvp_matrix = (proj * view).astype(np.float32)
    ...
    def set_line_width(self, width):
        self.line_width = max(0.1, width)  # Valor mínimo
        self.update()
